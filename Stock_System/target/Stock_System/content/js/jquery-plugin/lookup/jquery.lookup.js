    ...
    function setValuesHandle(target, value, queryText) {
        ...
        if (queryText) {
            if (value && options.url) {
                ...
                $.getJSON(rootPath + options.url, queryParams, function (d) {
...
//            var pPanel = parent.$(panel);
			var pPanel = $("<div></div>");
            options.window.content = "<iframe id='frm_win_" + options.valueField + "' src='" + rootPath + options.view + (options.view.indexOf('?') > -1 ? '&' : '?') + "r=" + Math.random() + "' style='height:100%;width:100%;border:0;' frameborder='0'></iframe>" //frameborder="0" for ie7
        ...
        if (!options.customShowPanel && options.lookupType) {    //防止访问url,参数不对，产生错误，自定义的不需要这个url
        	options.view = '/plugins/lookup.do';
            options.url = options.lookupType+'!getText.do';
